...
extension Piece {
    static func load_array(array: Array<JSON>) -> [Piece] {
        ...
        for each:JSON in array {
            ...
            pieces.append(Piece(pieceId: each["piece_id"].int!, pieceText: each["piece_text"].string!, pieceTime: each["piece_time"].string!.toDate(DateFormat.ISO8601)!, likeCnt: each["like_cnt"].int!, commentCnt: each["comment_cnt"].int!, user: user, channel: nil))
        }
        return pieces
    }
